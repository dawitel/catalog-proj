...
func TestProduct_NilReceiver(t *testing.T) {
	...
	assert.Nil(t, p.DomainEvents())
	assert.NoError(t, p.UpdateDetails("", "", "", time.Time{}))
	assert.NoError(t, p.Activate(time.Time{}))
	assert.NoError(t, p.Deactivate(time.Time{}))
	assert.NoError(t, p.Archive(time.Time{}))
	assert.NoError(t, p.ApplyDiscount(nil, time.Time{}))
	assert.NoError(t, p.RemoveDiscount(time.Time{}))
}
